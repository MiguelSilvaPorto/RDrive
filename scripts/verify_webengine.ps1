...
$binScript = Join-Path $Root 'scripts\_webengine_check_binaries.py'
...
$setHtmlScript = Join-Path $Root 'scripts\_webengine_sethtml_test.py'
...
if (-not $SkipNetwork) {
    $probeScript = Join-Path $Root 'scripts\_webengine_probe_test.py'
...
if ($ExitCode -ne 0) {
    ...
    Write-Step '3. Execute novamente: .\scripts\verify_webengine.ps1' 'WARN'
