...
from rdrive.core.cloud.terabox_setup import (
    TERABOX_MAIN_URL,
    TERABOX_RCLONE_PR_URL,
    merge_terabox_provider,
    open_terabox_login,
...
class AppService:
    ...
    def _cmd_list_providers(self, _args: dict[str, Any]) -> dict[str, Any]:
        ...
        for label, slug in entries:
            if not is_user_facing_provider(slug):
                continue
    ...
    def _cmd_get_settings(self, _args: dict[str, Any]) -> dict[str, Any]:
        window = self._require_window()
        settings = dict(window.settings or {})
        pid = window.config.profile_id
        settings["vault_enabled"] = ConfigStore.is_vault_enabled(pid)
        settings["vault_legacy_enc_present"] = (
            not settings["vault_enabled"] and ConfigStore.has_encrypted_vault(pid)
        )
    ...
    def _vault_unlock_snapshot(self, email: str | None = None) -> dict[str, Any]:
        from rdrive.core.vault.vault_unlock_flow import build_vault_unlock_ui_state
    ...
    def _cmd_unlock_vault(self, args: dict[str, Any]) -> dict[str, Any]:
        from rdrive.core.vault.vault_unlock_flow import apply_vault_unlock, validate_vault_unlock
    ...
    def _cmd_cancel_vault_unlock(self, _args: dict[str, Any]) -> dict[str, Any]:
        ...
        from rdrive.core.vault.vault_unlock_flow import build_vault_unlock_ui_state, clear_vault_unlock_pending

        window = self._require_window()
        ui = build_vault_unlock_ui_state()
        must_exit = bool(ui.get("required")) and bool(
            getattr(window, "_vault_unlock_pending", False)
        )

        if not must_exit:
            clear_vault_unlock_pending()
            window._vault_unlock_pending = False
            if not window._startup_checks_done:
                window._run_startup_checks()
            self.push_full_state()
            return {"ok": True, "vaultUnlock": ui}
    ...
    def _cmd_forgot_vault_password(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        from rdrive.core.profile.user_profile import is_valid_email, mask_email, set_active_profile, set_active_profile_default
    ...
    def _cmd_ping(self, _args: dict[str, Any]) -> dict[str, Any]:
        from rdrive.ui.terabox.terabox_browser import get_webengine_status, webengine_available

        webengine = get_webengine_status()
        return {
            "ok": True,
            "message": "pong",
            ...
            "features": {
                ...
                "teraboxEmbeddedBrowser": webengine_available(),
            },
            "webengine": webengine,
        }
    ...
    def _cmd_open_terabox_login(self, _args: dict[str, Any]) -> dict[str, Any]:
        url = open_terabox_login()
        self._log.info(f"[WEBUI] TeraBox: browser aberto para login ({url})")
        return {
            "ok": True,
            "url": url,
    ...
    def _cmd_open_terabox_embedded_browser(self, _args: dict[str, Any]) -> dict[str, Any]:
        ...
        if result.get("webengine_broken"):
            return {
                ...
                "error": str(result.get("error") or "PyQt6-WebEngine não instalado ou incompleto."),
                "hint": str(result.get("hint") or ""),
                "verify_script": str(result.get("verify_script") or ""),
                "fallback": True,
            }
        ...
        return {
            "ok": True,
            "cookie": cookie,
            "ndus": bool(result.get("ndus")),
            "main_url": TERABOX_MAIN_URL,
            "hint": str(
                result.get("hint")
                or "Cookie preenchido automaticamente — pode testar a ligação."
            ),
        }

    def _cmd_check_terabox_backend(self, _args: dict[str, Any]) -> dict[str, Any]:
        window = self._require_window()
        available = terabox_backend_available(window.rclone_cli)
        return {
            "ok": True,
            "available": available,
            "message": "" if available else terabox_backend_install_message(),
            "pr_url": TERABOX_RCLONE_PR_URL,
        }

    def _cmd_test_terabox_connection(self, args: dict[str, Any]) -> dict[str, Any]:
        window = self._require_window()
        guided_answers = args.get("guided_answers")
        if not isinstance(guided_answers, dict):
            guided_answers = {"cookie": str(args.get("cookie") or "")}
        cookie = str(guided_answers.get("cookie") or "").strip()
        ok_cookie, cookie_msg = validate_terabox_cookie(cookie)
        if not ok_cookie:
            return {"ok": False, "message": cookie_msg}
        if not terabox_backend_available(window.rclone_cli):
            return {"ok": False, "message": terabox_backend_missing_message()}
    ...
    def _cmd_open_logs_folder(self, _args: dict[str, Any]) -> dict[str, Any]:
        from rdrive.core.logging.app_logger import open_logs_folder
