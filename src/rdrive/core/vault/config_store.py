...
from rdrive.core.profile.session_store import clear_remembered
from rdrive.core.vault.vault import Vault
...
class ConfigStore:
    ...
    def load_settings(self) -> dict[str, Any]:
        ...
        # profile_meta.json is authoritative; ignore stale vault_enabled inside JSON/.enc.
        settings["vault_enabled"] = bool(
    ...
    def _load_json(self, path: Path, default: Any) -> Any:
        ...
        if self._vault:
            try:
                ...
                from rdrive.core.logging.human_log import log_exception_event
        ...
        try:
            ...
            get_app_logger().log_exception(f"json_load:{path.name}", exc)
            log_exception_event(f"Ficheiro local inválido ({path.name})", exc)
            return default
    ...
    def default_profile_meta(cls) -> dict[str, Any]:
        return {"v": _PROFILE_META_VERSION, "vault_enabled": False}
    ...
    def save_profile_meta(cls, meta: dict[str, Any], profile_id: str | None = None) -> None:
        ...
        payload["vault_enabled"] = bool(meta.get("vault_enabled", False))
    ...
    def is_vault_enabled(cls, profile_id: str | None = None) -> bool:
        """Whether the encrypted vault is required for this profile (explicit opt-in only)."""
        pid = resolve_profile_id(profile_id=profile_id)
        return bool(cls.load_profile_meta(pid).get("vault_enabled", False))
    ...
    def enable_vault(self, new_password: str) -> None:
        ...
        if vault_state == VaultState.ENCRYPTED:
            ok, validation_error = self.verify_vault_password(new_password, profile_id=pid)
            if not ok:
                raise ValueError(
                    validation_error
                    or "Senha mestra incorreta para reactivar o cofre existente (.enc)."
                )
            ...
            settings_payload = self._load_json_strict(
                self.settings_path, self.load_settings(), self._vault
            )
            ...
            get_app_logger().info(
                "[VAULT] re-enabled — existing .enc files",
                module="config_store",
            )
            log_user_event(
                "Segurança",
                "Cofre reactivado — ficheiros .enc existentes",
