    ...
    "terabox": [
        {
            ...
            "type": "password",
            "required": True,
            "placeholder": "ndus=… ou Cookie: completo do browser",
            "help": (
                "F12 → Rede → pedido a terabox.com → copie o cabeçalho Cookie "
                "(ou só o valor ndus=). Requer rclone não oficial com backend terabox."
            ),
        },
    ],
}
...
def build_guided_rclone_options(
    ...
    if backend == "s3":
        ...
        from rdrive.core.terabox_setup import build_terabox_rclone_options, normalize_terabox_cookie
...
def provider_connection_guidance(provider_slug: str) -> str:
    ...
    if backend == "terabox":
        return (
            "TeraBox (experimental): requer build rclone não oficial com backend terabox "
            "e cookie de sessão do browser (ndus). Cole o cookie no assistente RDrive."
        )
    if backend in _GUIDED_BACKENDS:
        return (
