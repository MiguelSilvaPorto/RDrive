...
_BUNDLED_RELATIVE = Path("tools") / "rclone-extra" / "rclone.exe"
...
def bundled_rclone_path(project_root: Path | None = None) -> Path | None:
    """``tools/rclone-extra/rclone.exe`` no root do projeto, se existir."""
    root = project_root or resolve_project_root()
    candidate = root / _BUNDLED_RELATIVE
...
def is_bundled_rclone_executable(executable: str) -> bool:
    if executable.strip().lower() == "rclone":
        return False
    try:
    ...
    if bundled is not None and resolved == bundled:
        return True
    return "rclone-extra" in resolved.parts


def rclone_version_probe_timeout(executable: str) -> int:
    """Timeout de ``rclone version`` — maior para binário local (cold start / AV)."""
    if is_bundled_rclone_executable(executable):
        return _BUNDLED_RCLONE_PROBE_TIMEOUT_SEC
    if executable.strip().lower() != "rclone":
        return _BUNDLED_RCLONE_PROBE_TIMEOUT_SEC
    return _PATH_RCLONE_PROBE_TIMEOUT_SEC


def rclone_availability_user_message(exc: RcloneError, executable: str) -> tuple[str, str]:
    """Título e corpo em pt-BR para falha de disponibilidade do rclone."""
    detail = str(exc).strip()
    lower = detail.lower()
    exe_display = executable
    if executable.strip().lower() != "rclone":
        exe_display = str(Path(executable).expanduser())

    if "demorou demais" in lower or "timeout" in lower:
        timeout_sec = rclone_version_probe_timeout(executable)
        body = (
            "O RDrive encontrou o executável do rclone, mas ele não respondeu a tempo.\n\n"
            f"Executável: {exe_display}\n"
            f"Limite: {timeout_sec}s\n\n"
            "Causas comuns no Windows:\n"
            "• Primeira execução (antivírus ou Defender a analisar o ficheiro)\n"
            "• Disco ou CPU muito ocupados\n\n"
            "O que fazer:\n"
    ...
    if "executável do rclone não encontrado" in lower or "executavel do rclone nao encontrado" in lower:
        body = (
            "O caminho configurado para o rclone não existe ou não é executável.\n\n"
            f"Caminho: {exe_display}\n\n"
            "O que fazer:\n"
    ...
    if "não encontrado no path" in lower or "nao encontrado no path" in lower:
        body = (
            "O RDrive precisa do rclone instalado e acessível.\n\n"
            "No Windows, execute num terminal novo:\n"
            "winget install --id Rclone.Rclone -e --scope user\n\n"
            "Para TeraBox, use o build em tools\\rclone-extra\\ (Iniciar.bat).\n"
    ...
    body = (
        "Não foi possível validar o rclone.\n\n"
        f"Executável: {exe_display}\n\n"
        f"Detalhe técnico: {detail}"
    )
    return "rclone indisponivel", body
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            if self.executable.strip().lower() != "rclone":
                raise RcloneError(
                    f"Executável do rclone não encontrado: {self.executable}"
                ) from exc
            raise RcloneError(
