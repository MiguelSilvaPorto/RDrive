"""Detached RDrive process restart (watchdog / settings / CTk)."""
...
def _schedule_callback(delay_ms: int, callback: Callable[[], None]) -> None:
    """Schedule on Qt main thread when available, else a daemon timer (CTk)."""
    try:
        ...
        pass
    timer = threading.Timer(max(0.05, delay_ms / 1000.0), callback)
    timer.daemon = True
    timer.start()


def _show_spawn_error(message: str, *, parent: object | None = None) -> None:
    extra = (
        "\n\nSe o RDrive não voltar a abrir, feche processos «pythonw» antigos "
        "no Gerenciador de Tarefas e use Iniciar.bat."
    )
    ...
    try:
        ...
        if QApplication.instance() is not None:
            QMessageBox.critical(None, "Reiniciar RDrive", text)
            return
    except Exception:  # noqa: BLE001
        pass
    try:
        ...
        messagebox.showerror("Reiniciar RDrive", text, parent=parent)  # type: ignore[arg-type]
    except Exception:  # noqa: BLE001
        pass


def _spawn_via_ps1(project_root: Path, env: dict[str, str]) -> tuple[bool, str]:
    """Primary path: scripts/maintenance/restart_rdrive.ps1 (Start-Process, same as Iniciar.bat)."""
    script = project_root / "scripts" / "maintenance" / "restart_rdrive.ps1"
...
def _begin_restart_handoff() -> bool:
    """Mark intentional restart and release single-instance lock."""
...
def _schedule_restart_sequence(
    project_root: Path,
    *,
    ...
    on_restart_stalled: Callable[[], None] | None,
    error_parent: object | None,
) -> bool:
    ...
    def _fail_restart(reason: str, *, show_dialog: bool = True) -> None:
        ...
        if show_dialog:
            _show_spawn_error(reason, parent=error_parent)
    ...
    def _spawn() -> None:
        if start_detached_rdrive(
            ...
            _schedule_callback(_SPAWN_CHILD_VERIFY_MS, _verify_child_process)
            return
    ...
    def _quit() -> None:
        ...
        try:
            ...
            _restart_trace(f"quit_callback failed: {exc}", level="warning")
        try:
            from PyQt6.QtWidgets import QApplication
    ...
    _schedule_callback(spawn_ms, _spawn)
    _schedule_callback(quit_ms, _quit)
    _schedule_callback(_RESTART_VERIFY_MS, _verify_restart)
    _restart_trace(f"scheduled spawn={spawn_ms}ms quit={quit_ms}ms verify={_RESTART_VERIFY_MS}ms")
    return True


def request_rdrive_restart_ctk(
    project_root: Path,
    *,
    quit_callback: Callable[[], None],
    ...
    """Release single-instance, spawn a fresh RDrive, then quit (CustomTkinter / no Qt)."""
    if not _begin_restart_handoff():
        return False
    return _schedule_restart_sequence(
        project_root,
        quit_callback=quit_callback,
        quit_delay_ms=quit_delay_ms,
        spawn_delay_ms=spawn_delay_ms,
        clear_session_keys=clear_session_keys,
        on_spawn_failed=on_spawn_failed,
        on_restart_stalled=on_restart_stalled,
...
def request_rdrive_restart(
    ...
    *,
    ...
    """Release single-instance, spawn a fresh RDrive, then quit gracefully (PyQt)."""
    try:
        from PyQt6.QtWidgets import QApplication
    except ImportError:
        return False
