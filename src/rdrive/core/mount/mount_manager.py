...
from rdrive.core.runtime.subprocess_utils import (
    format_command,
    log_popen_failure,
    popen_logged,
    run_logged,
)
from rdrive.core.mount.shared_mount import SharedMountValidationError, build_mount_target
...
def _read_mount_log_since_offset(log_file: Path, offset: int, *, max_chars: int = 1200) -> str:
    """Return tail of mount.log written after *offset* bytes (avoids stale session noise)."""
    if not log_file.is_file() or offset < 0:
        return ""
    try:
    ...
    try:
        with log_file.open("rb") as handle:
            handle.seek(offset)
            chunk = handle.read(max(size - offset, max_chars + 4096))
    except OSError:
        return ""
...
def _scrub_mount_log_noise(text: str) -> str:
    """Drop benign RC teardown lines that confuse users after a failed mount."""
    kept: list[str] = []
    for line in text.splitlines():
        low = line.lower()
        if "mount/unmount" in low and "mount not found" in low:
            continue
        if "unmounted rclone mount" in low:
            continue
        if "serving remote control on" in low:
            continue
        kept.append(line)
    return "\n".join(kept).strip()


def _parse_rc_port_from_commandline(commandline: str) -> int | None:
    match = _RC_ADDR_RE.search(commandline or "")
    if match:
        try:
            return int(match.group(1))
        except ValueError:
            return None
    if "--rc" in (commandline or "").lower():
        return _DEFAULT_RC_PORT
    return None
...
def _format_mount_failure(detail: str, log_file: Path, *, log_offset: int = 0) -> str:
    combined = detail.strip()
    log_tail = _scrub_mount_log_noise(
        _read_mount_log_since_offset(log_file, log_offset)
        if log_offset > 0
        else _read_mount_log_tail(log_file)
    )
...
def _try_rclone_rc_unmount(
    ...
    *,
    ...
    try:
        ...
        ok = completed.returncode == 0 or "mount not found" in stderr
        logger.info(
...
def _find_rclone_mount_processes(mountpoint: str) -> list[tuple[int, str]]:
    """Return (pid, commandline) for orphan ``rclone mount`` on *mountpoint*."""
    token = _mountpoint_token(mountpoint)
    ...
    for line in (completed.stdout or "").splitlines():
        ...
        if not pid_text.isdigit():
            continue
        cmdline = parts[1].strip() if len(parts) > 1 else _get_process_commandline(int(pid_text))
        processes.append((int(pid_text), cmdline))
    return processes


def _kill_orphan_rclone_mounts(
    mountpoint: str,
    *,
    label: str,
    rclone_executable: str | None = None,
) -> None:
    """Stop orphan rclone mounts: RC unmount/quit when port known, then taskkill."""
    logger = get_app_logger()
    token = _mountpoint_token(mountpoint)
    for pid, cmdline in _find_rclone_mount_processes(mountpoint):
        rc_port = _parse_rc_port_from_commandline(cmdline)
        if rc_port is not None and rclone_executable and token:
            logger.info(
                f"[MOUNT] stale cleanup step=rclone_rc pid={pid} port={rc_port} mountpoint={mountpoint} drive={label}",
                module="mount",
            )
            _try_rclone_rc_unmount(rclone_executable, rc_port, mountpoint, label=label)
            _try_rclone_rc_quit(rclone_executable, rc_port, label=label)
        logger.info(
    ...
    def connect(self, drive: Drive, *, mount_as_local_drive: bool = True) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if returncode is not None:
                ...
                detail = self._augment_mount_failure(drive, remote, detail)
                log_popen_failure(args, context="mount", returncode=returncode, stderr=detail)
                logger.error(
                    f"[MOUNT] process exited drive={label} code={returncode} remote={remote}",
                    module="mount",
                )
        ...
        raise MountError(self._augment_mount_failure(drive, remote, detail))

    def _augment_mount_failure(self, drive: Drive, remote: str, detail: str) -> str:
        """Add pt-BR hints for common misconfiguration (TeraBox binary, letter conflict)."""
        ...
        if "google drive root" in lowered and is_terabox_provider(backend):
            lines.append(
                "O registo de log parece ser de outra unidade (Google Drive). "
                "Confirme que o remote desta unidade é TeraBox (ex.: terabox_pessoal) e não um remote Google Drive."
            )
        if remote and remote not in detail:
            lines.append(f"Remote usado no mount: {remote}")
        if not lines:
            return detail
        prefix = "\n\n".join(lines)
        return f"{prefix}\n\n{detail}".strip()
